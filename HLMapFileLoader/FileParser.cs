    ...
    public class FileParser
    {
        ...
        private static Face ParseFace(string brushData)
        {
            ...
            Vector3 p1 = new Vector3(float.Parse(brushDataP1.Split(' ')[0]),
                ...
                float.Parse(brushDataP1.Split(' ')[2], NumberStyles.Any, ci)) / scale;

            Vector3 p2 = new Vector3(float.Parse(brushDataP2.Split(' ')[0]),
                float.Parse(brushDataP2.Split(' ')[1], NumberStyles.Any, ci),
                float.Parse(brushDataP2.Split(' ')[2], NumberStyles.Any, ci)) / scale; ;
            ...
            Vector3 p3 = new Vector3(float.Parse(brushDataP3.Split(' ')[0], NumberStyles.Any, ci),
                ...
                float.Parse(brushDataP3.Split(' ')[2], NumberStyles.Any, ci)) / scale; ;
            ...
            float textureScaleU = float.Parse(textureScale.Split(' ')[1].Trim(), NumberStyles.Any, ci) / scale; ;
            float textureScaleV = float.Parse(textureScale.Split(' ')[2], NumberStyles.Any, ci) / scale; ;
